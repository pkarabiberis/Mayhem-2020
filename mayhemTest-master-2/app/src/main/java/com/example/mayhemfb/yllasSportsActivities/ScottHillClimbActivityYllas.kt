...
class ScottHillClimbActivityYllas : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_hillclimb_yllas)
        ...
        val hillClimb = yllas.collection("Sports").document("scott-hillclimb-series")
        val sportTime = yllas.collection("Timetables").document("scott-hillclimb-series")
        ...
        hillClimb.collection("results")
            .orderBy("time", Query.Direction.ASCENDING)
            .get()
            .addOnSuccessListener { documents ->
                if (documents.isEmpty) {
                    val emptyScoreText = "No results yet."
                    emptyScore_hillClimb_yllas.text = emptyScoreText
                } else {
                    for (title in captions) {
                        ...
                        captionsLayoutHillclimb_yllas.addView(caption)
                    }
