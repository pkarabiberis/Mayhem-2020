...
import kotlinx.android.synthetic.main.activity_pimp_yllas.*
import kotlinx.android.synthetic.main.activity_poolcrossing_yllas.*
...
class PoolCrossingActivityYllas : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_poolcrossing_yllas)
        ...
        val poolCrossing = yllas.collection("Sports").document("pool-crossing")
        val sportTime = yllas.collection("Timetables").document("pool-crossing")
        ...
        poolCrossing.collection("results")
            .orderBy("time", Query.Direction.ASCENDING)
            .get()
            .addOnSuccessListener { documents ->
                if (documents.isEmpty) {
                    val emptyScoreText = "No results yet."
                    emptyScore_pool_yllas.text = emptyScoreText
                } else {
                    for (title in captions) {
                        ...
                        captionsLayoutPool_yllas.addView(caption)
                    }
