...
class PimpMyPulkkaActivityYllas : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_pimp_yllas)
        ...
        val pimpMyPulkka = yllas.collection("Sports").document("pimp-my-pulkka")
        val sportTime = yllas.collection("Timetables").document("pimp-my-pulkka")
        ...
        pimpMyPulkka.collection("results")
            .orderBy("time", Query.Direction.ASCENDING)
            .get()
            .addOnSuccessListener { documents ->
                if (documents.isEmpty) {
                    val emptyScoreText = "No results yet."
                    emptyScore_pimp_yllas.text = emptyScoreText
                } else {
                    for (title in captions) {
                        ...
                        captionsLayoutPimp_yllas.addView(caption)
                    }
