...
class BankedSlalomYllas : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_banked_slalom_yllas)
        ...
        val bankedSlalom = yllas.collection("Sports").document("banked-slalom-xl")
        val sportTime = yllas.collection("Timetables").document("banked-slalom-xl")
        ...
        bankedSlalom.collection("results")
            .orderBy("time", Query.Direction.ASCENDING)
            .get()
            .addOnSuccessListener { documents ->
                if (documents.isEmpty) {
                    val emptyScoreText = "No results yet."
                    emptyScore_banked_yllas.text = emptyScoreText
                } else {
                    for (title in captions) {
                        ...
                        captionsLayoutBanked_yllas.addView(caption)
                    }
