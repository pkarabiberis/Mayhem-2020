  ...
  <v-container>
    <v-row>
      <v-col cols="12">
        ...
        <p class="display-3 text-center">Luosto</p>
        <hr />
      </v-col>
    </v-row>
          ...
          <div>
            ...
            <v-list class="transparent">
              <v-list-item v-for="time in timetables" :key="time.id">
                <v-list-item-title class="mx-10">{{ time.content }}</v-list-item-title>
              ...
              <v-row justify="center">
                <v-dialog v-model="dialog" max-width="500px">
                  ...
                  <v-card>
                    ...
                    <v-card-actions>
                      <v-btn text @click="addContent(content, start, end)">Save</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-dialog>
                <v-dialog v-model="editDialog" max-width="500px">
                  <v-card>
                    ...
                    <v-card-actions>
                      <v-btn
                        text
                        @click="editContent(newContent, newStartTime, newEndTime, testeriId)"
                      >Save</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-dialog>
...
export default {
  data: () => ({
    sports: [
      ...
      {
        name: 'Snowmobile Drag Race',
        img: require('../../public/images/snowmobile_drag_race_mayhem.jpeg')
      }
    ],
    timetables: [],
    content: null,
  ...
  methods: {
    ...
    getContent(id) {
      ...
      firebase
        ...
        .then(doc => {
          if (doc.exists) {
            const { content, startTime, endTime } = doc.data();
    ...
    editContent(content, startTime, endTime, id) {
      firebase
        .firestore()
        .collection('Locations')
        .doc('Luosto')
